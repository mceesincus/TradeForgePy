...
# REFINED and CORRECTED version - adds missing errorCode 10 to TSLoginErrorCode
...
MODEL_CONFIG_TS = ConfigDict(populate_by_name=True, extra='ignore', arbitrary_types_allowed=True)

# --- Enumerations from OpenAPI Spec (with corrections) ---

class TSSearchAccountErrorCode(IntEnum):
    SUCCESS = 0
...
class TSLoginErrorCode(IntEnum):
    ...
    ACCOUNT_REJECTED = 10  # <-- ADDED THIS MISSING ERROR CODE

class TSLogoutErrorCode(IntEnum):
    SUCCESS = 0
    INVALID_SESSION = 1
    UNKNOWN_ERROR = 2

class TSValidateErrorCode(IntEnum):
    SUCCESS = 0
    INVALID_SESSION = 1
    SESSION_NOT_FOUND = 2
    EXPIRED_TOKEN = 3
...
class TSSearchContractErrorCode(IntEnum):
    SUCCESS = 0

class TSSearchContractByIdErrorCode(IntEnum):
    SUCCESS = 0
    CONTRACT_NOT_FOUND = 1

class TSRetrieveBarErrorCode(IntEnum):
    SUCCESS = 0
    CONTRACT_NOT_FOUND = 1

class TSAggregateBarUnit(IntEnum):
    UNSPECIFIED = 0
    SECOND = 1
    MINUTE = 2
    HOUR = 3
...
class TSSearchOrderErrorCode(IntEnum):
    SUCCESS = 0
    ACCOUNT_NOT_FOUND = 1

class TSOrderStatus(IntEnum):
    NONE = 0
    OPEN = 1
    FILLED = 2
    CANCELLED = 3
...
class TSTraderOrderType(IntEnum):
    UNKNOWN = 0
    LIMIT = 1
    MARKET = 2
    STOP_LIMIT = 3
...
class TSCancelOrderErrorCode(IntEnum):
    SUCCESS = 0
    ACCOUNT_NOT_FOUND = 1
    ORDER_NOT_FOUND = 2
    REJECTED = 3
...
class TSModifyOrderErrorCode(IntEnum):
    SUCCESS = 0
    ACCOUNT_NOT_FOUND = 1
    ORDER_NOT_FOUND = 2
    REJECTED = 3
...
class TSSearchPositionErrorCode(IntEnum):
    SUCCESS = 0
    ACCOUNT_NOT_FOUND = 1

class TSPositionType(IntEnum):
    UNDEFINED = 0
    LONG = 1
    SHORT = 2
...
class TSSearchTradeErrorCode(IntEnum):
    SUCCESS = 0
    ACCOUNT_NOT_FOUND = 1
...
class TSRetrieveBarRequest(BaseModel): model_config = MODEL_CONFIG_TS; contractId: str; live: bool; startTime: datetime; endTime: datetime; unit: TSAggregateBarUnit; unitNumber: int; limit: int; includePartialBar: bool
class TSSearchOrderRequest(BaseModel): model_config = MODEL_CONFIG_TS; accountId: int; startTimestamp: datetime; endTimestamp: Optional[datetime] = None
...
class TSSearchTradeRequest(BaseModel): model_config = MODEL_CONFIG_TS; accountId: int; startTimestamp: Optional[datetime] = None; endTimestamp: Optional[datetime] = None
